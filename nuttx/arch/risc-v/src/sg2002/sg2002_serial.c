...
#define SG2002_UART3_BASE           0x04170000
...
struct dw_regs {
	...
	volatile reg_type spr; /* 0x20 Scratch Register */
};
...
void sg2002_debug_log(const char *s);
...
{
    if ((base == SG2002_UART_SIM_BASE) || \
        (base == SG2002_UART0_BASE) || \
        (base == SG2002_UART1_BASE) || \
        (base == SG2002_UART2_BASE) || \
        (base == SG2002_UART3_BASE))
        return true;
...
static inline void sg2002_disableuartint(struct uart_dev_s *dev)
{
    ...
    if ((priv == NULL) || \
        !sg2002_check_uart_base(priv->uartbase))
        return;
